...
class DataBase {
    ...
    void balance_children(Page &root, Page &left, Page &right);
    void merge_children(Page &root, Page &left, Page &right);
    
    
    int search_rec(int dpth, int pg, const std::string &key, std::string &val);
    ...
    bool delete_deb(const std::string &key);
    
    int delete_safe(const std::string &key);
    /*удаляет ключ из дерева, возможно подменяя корень в случае баллансировки*/
    
    int delete_rec(int dpth, int pg, const std::string &key);
    /*удалает ключ из дерева, но не баллансирует корневую вершину*/
